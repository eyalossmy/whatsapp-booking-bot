...
async function syncCalendarForBusiness(business) {
  ...
  try {
    ...
    // ── בטל תורים שנמחקו מהיומן ──
    const { data: dbAppts } = await supabase
      .from('appointments')
      .select('id, google_event_id, customer_name')
    ...
    for (const appt of (dbAppts || [])) {
      if (!calendarEventIds.has(appt.google_event_id)) {
        await supabase
          .from('appointments')
          .update({ status: 'cancelled', cancelled_at: new Date().toISOString() })
          .eq('id', appt.id);
        console.log(`🗑️ Cancelled (deleted from Calendar): ${appt.customer_name} [${appt.id}]`);
      }
    }
