...
async function findFreeSlots(business, fromDate, count = 6, preferredHour = null) {
  ...
  // אם יש שעה מועדפת - מיין לפי קרבה לשעה המבוקשת
  if (preferredHour !== null) {
    allSlots.sort((a, b) => {
      const aDiff = Math.abs(a.getHours() + a.getMinutes()/60 - preferredHour);
      const bDiff = Math.abs(b.getHours() + b.getMinutes()/60 - preferredHour);
      if (Math.abs(aDiff - bDiff) > 0.01) return aDiff - bDiff;
      return a - b; // אותה שעה? מוקדם יותר קודם
    });
  }
...
async function processWithAI(messageText, customerPhone, business) {
  try {
    ...
    // חלץ שעה מבוקשת מההודעה האחרונה של הלקוח
    const hourMatch = messageText.match(/(\d{1,2})(?:[:.]?(\d{2}))?/);
    const preferredHour = hourMatch ? parseInt(hourMatch[1]) + (parseInt(hourMatch[2] || 0) / 60) : null;
...
2. כשהלקוח מבקש זמן:
   א. אם הזמן המבוקש מופיע ברשימת הזמנים הפנויים → שאל אישור מיד.
   ב. אם לא מופיע → בדוק ברשימת התפוסים. אם שם → "תפוס" + הצע 3 זמנים קרובים מהרשימה הפנויה.
   ג. אסור לומר "תפוס" על זמן שמופיע ברשימת הפנויים!
