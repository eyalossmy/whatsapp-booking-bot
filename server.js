...
async function notifyBusinessOwner(business, appointment, customerPhone, action = 'new') {
  ...
  // ✅ הצג מספר טלפון בלי קידומת ישראל
  const displayPhone = customerPhone.replace(/^\+?972/, '0');
...
async function processWithOpenAI(messageText, customerPhone, business) {
  try {
    ...
    // ✅ תיקון: מציג גם תצוגה עברית וגם ISO כדי ש-AI יוכל להשתמש ב-CONFIRM נכון
    const availableTimesText = availableSlots.map(s => {
      const hebrewDisplay = s.toLocaleString('he-IL', {
        weekday: 'long',
        day: 'numeric',
        month: 'numeric',
        hour: '2-digit',
      ...
      const isoDate = s.toISOString().slice(0, 19); // YYYY-MM-DDTHH:mm:ss
      return `${hebrewDisplay} [ISO:${isoDate}]`;
    }).join('\n');
...
כל שורה מכילה תצוגה עברית ואת קוד ה-ISO לשימוש ב-CONFIRM:
${availableTimesText || 'אין זמנים פנויים בשבוע הקרוב'}

${contextInfo}

⚠️ חוקי קביעת תור - חובה:

1. שם מלא - חובה:
   תשאל "מה השם המלא שלך?" ואל תמשיך בלי שם פרטי + משפחה.
   לא לקבל "א." או "שם" סתם - תבקש שם מלא.

2. כשהלקוח מבקש זמן שנמצא ברשימת הזמנים הפנויים:
   → שאל אישור עם תאריך מלא. אל תציע אלטרנטיבות!

3. כשהלקוח מבקש זמן שאינו ברשימה:
   → באותה הודעה: אמור שלא פנוי + הצע מספר זמנים מהרשימה.
   דוגמה: "יום שלישי 12:00 לא פנוי. הזמנים הפנויים הקרובים:
   • יום שלישי 13:00
   • יום רביעי 09:00
   • יום רביעי 10:00
   איזה מהם מתאים?"
...
4. ⚠️ זיכרון הצעות:
   אם הצעת "13:00 או 14:00" והלקוח ענה "13:00" → אל תשאל "איזה יום?", תאשר!

5. לפני CONFIRM - חובה:
   "מאשר לך תור ל[יום], [תאריך] בשעה [שעה]?"

6. רק אחרי "כן" / "אישור" → כתוב בשורה נפרדת:
   CONFIRM:[ISO מהרשימה]|NAME:שם_מלא_הלקוח

⚠️ ה-ISO חייב להיות מהרשימה למעלה - העתק אותו בדיוק!
⚠️ אסור CONFIRM לפני אישור מפורש!

ביטול: CANCEL:[id]
שינוי: RESCHEDULE:[id]|NEW_TIME:[ISO]|NAME:[שם]
